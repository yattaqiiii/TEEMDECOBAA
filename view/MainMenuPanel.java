...
public class MainMenuPanel extends JPanel {
    ...
    public MainMenuPanel(GameView parentFrame) {
        ...
        scoreTable.setFont(customFont.deriveFont(16f));
    ...
    private JPanel createLeftPanel() {
        JPanel leftPanel = new JPanel(new GridBagLayout()); leftPanel.setOpaque(false); GridBagConstraints leftGbc = new GridBagConstraints();
        JTextField nameField = new JTextField(15); nameField.setFont(customFont.deriveFont(18f)); nameField.setHorizontalAlignment(JTextField.CENTER);
        JButton startButton = createImageButton("/assets/start_button.png");
        JButton quitButton = createImageButton("/assets/quit_button.png");
        easyButton = new JRadioButton("Easy", true); mediumButton = new JRadioButton("Medium"); hardButton = new JRadioButton("Hard");
        setupRadioButton(easyButton); setupRadioButton(mediumButton); setupRadioButton(hardButton);
        ButtonGroup difficultyGroup = new ButtonGroup(); difficultyGroup.add(easyButton); difficultyGroup.add(mediumButton); difficultyGroup.add(hardButton);
        JLabel usernameLabel = new JLabel("Username:"); setupLabel(usernameLabel, 18f);
        JLabel modeLabel = new JLabel("Mode:"); setupLabel(modeLabel, 18f);

        leftGbc.fill = GridBagConstraints.HORIZONTAL; leftGbc.insets = new Insets(0, 0, 5, 0);
        leftGbc.gridx = 0; leftGbc.gridy = 0; leftPanel.add(usernameLabel, leftGbc);
        leftGbc.gridy++; leftPanel.add(nameField, leftGbc);
        leftGbc.gridy++; leftGbc.insets = new Insets(10, 0, 10, 0); leftPanel.add(startButton, leftGbc);
        leftGbc.gridy++; leftGbc.insets = new Insets(20, 0, 5, 0); leftPanel.add(modeLabel, leftGbc);
        leftGbc.gridy++; leftGbc.insets = new Insets(0, 0, 0, 0); leftPanel.add(easyButton, leftGbc);
        leftGbc.gridy++; leftPanel.add(mediumButton, leftGbc); leftGbc.gridy++; leftPanel.add(hardButton, leftGbc);
        leftGbc.gridy++; leftGbc.weighty = 1.0; leftGbc.anchor = GridBagConstraints.SOUTH; leftPanel.add(quitButton, leftGbc);
    ...
    private void loadAssets() {
        ...
        try (InputStream is = getClass().getResourceAsStream("/assets/Daydream.ttf")) {
            ...
            System.err.println("Gagal memuat Daydream.ttf: " + e.getMessage());
            customFont = new Font("Arial", Font.BOLD, 14);
            titleFont = new Font("Arial", Font.BOLD, 30);
        }
    }

    private void setupRadioButton(JRadioButton button) { button.setFont(customFont.deriveFont(20f)); button.setOpaque(false); button.setForeground(FONT_COLOR); button.setFocusPainted(false); }
